...
void FST::FST_linecheck(char* someline, LOG log, int num) {
	FST myFST(					// нка a(b)*(cb^+(d|e)f)^+b*g
	...
	if (execute(myFST)) {
		std::cout << "Цепочка " << myFST.string << " распознана." << std::endl << std::endl;
		*log.stream << "Цепочка " << myFST.string << " распознана." << std::endl;
	}
	else {
		std::cout << "Цепочка " << myFST.string << " не распознана." << std::endl;
		std::cout << "ОШИБКА: строка " << num << ", позиция " << (myFST.position + 1) << std::endl << std::endl;
		*log.stream << "Цепочка " << myFST.string << " не распознана." << std::endl;
		*log.stream << "ОШИБКА: строка " << num << ", позиция " << (myFST.position + 1) << std::endl << std::endl;
